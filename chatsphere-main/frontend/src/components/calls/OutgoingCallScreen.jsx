...
export default function OutgoingCallScreen({ call, soundBlocked, onEnableSound, onCancel }) {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(10,132,255,0.22),transparent_30%),radial-gradient(circle_at_bottom,rgba(48,209,88,0.1),transparent_34%),linear-gradient(180deg,rgba(0,0,0,0.88),rgba(0,0,0,0.98))]" />
      ...
      >
        ...
        <motion.div className="flex flex-1 flex-col items-center justify-center text-center" variants={itemVariants}>
          <motion.div className="call-avatar-ring relative mb-6" initial={{ scale: 0.92 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 110 }}>
            <motion.div className="absolute inset-0 rounded-full bg-sky-400/20 blur-2xl" animate={{ opacity: [0.35, 0.8, 0.35], scale: [1, 1.08, 1] }} transition={{ duration: 1.7, repeat: Infinity, ease: 'easeInOut' }} />
            <Avatar src={call.peerUser?.avatar} name={peerName} size="xl" />
          </motion.div>

          <div className="call-status-chip mb-3 inline-flex items-center gap-2">
            <span className="h-2 w-2 rounded-full bg-[var(--wa-info)] animate-pulse" />
            Ringing
          </div>
          <p className="text-[11px] uppercase tracking-[0.35em] text-[var(--wa-text-secondary)]">Dialing {isVideo ? 'video' : 'voice'} call</p>
          <h1 className="mt-3 text-3xl font-semibold text-white sm:text-4xl">{peerName}</h1>
          <p className="mt-3 max-w-sm text-sm text-[var(--wa-text-secondary)]">Waiting for the other person to answer. Keep the app open for the most stable connection.</p>
          {soundBlocked ? (
            <motion.button
        ...
        <motion.div className="flex w-full items-center justify-center pt-2" variants={itemVariants}>
          <motion.button
